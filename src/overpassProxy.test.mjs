...
/** Set OVERPASS_MIRRORS for the test and restore it afterwards. */
async function withMirrorsEnv(value, run) {
  const original = process.env.OVERPASS_MIRRORS;
  if (value === undefined) delete process.env.OVERPASS_MIRRORS;
  else process.env.OVERPASS_MIRRORS = value;
  try {
    await run();
  } finally {
    if (original === undefined) delete process.env.OVERPASS_MIRRORS;
    else process.env.OVERPASS_MIRRORS = original;
  }
}

test('resolveOverpassUpstreams reads OVERPASS_MIRRORS lazily and normalizes it', async () => {
  await withMirrorsEnv(
    'overpass.private.coffee, https://overpass.kumi.systems/api/interpreter ,not a url',
    () => {
      // Hosts become full interpreter URLs; unparseable entries are skipped;
      // duplicates collapse; env is read per call, never at import time.
...
test('empty or garbage OVERPASS_MIRRORS falls back to the built-in mirrors', async () => {
  await withMirrorsEnv(undefined, () => {
    assert.deepEqual(resolveOverpassUpstreams(), BUILT_IN_MIRRORS);
  });
  await withMirrorsEnv(' , ,, ', () => {
    assert.deepEqual(resolveOverpassUpstreams(), BUILT_IN_MIRRORS);
  });
...
test('a configured mirror list drives the rotation without passing endpoints', async () => {
  await withMirrorsEnv('https://solo.example/api/interpreter', async () => {
    const m = mirrors({
      'https://solo.example/api/interpreter': { status: 200, body: DATA.body },
      'https://overpass-api.de/api/interpreter': {
        status: 200,
        body: DATA.body,
      },
    ...
    assert.equal(payload.endpoint, 'https://solo.example/api/interpreter');
    assert.deepEqual(
      m.tried,
      ['https://solo.example/api/interpreter'],
      'the env-configured list replaces the built-ins entirely',
    );
  });
});
